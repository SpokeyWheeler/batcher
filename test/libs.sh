...
pgruntests() {
	...
	/tmp/batcher update -concurrency 4 -database batchertestdb -dbtype postgres -host localhost -opts $opts -password btest -portnum $portnum -table serialtest -set "strcol='b'" -user btest -where "strcol='a'" -execute

	sera=$( $SQLCMD "SELECT COUNT(1) FROM serialtest WHERE strcol = 'a';" | grep -iv count | grep -iv row | tr -d '\r' )
	comp "Updated serial a" "$expa" "$sera"

	/tmp/batcher delete -concurrency 4 -database batchertestdb -dbtype postgres -host localhost -opts $opts -password btest -portnum $portnum -table serialtest -user btest -where "intcol<101" -execute

	sertot=$( $SQLCMD "SELECT COUNT(1) FROM serialtest;" | grep -iv count | grep -iv row | tr -d '\r' )
	comp "Small delete serial total" "$exptot" "$sertot"

	/tmp/batcher update -concurrency 4 -database batchertestdb -dbtype postgres -host localhost -opts $opts -password btest -portnum $portnum -set "strcol='b'"  -table uuidtest -user btest -where "strcol='a'" -execute

	uida=$( $SQLCMD "SELECT COUNT(1) FROM uuidtest WHERE strcol = 'a';" | grep -iv count | grep -iv row | tr -d '\r' )
	comp "Updated UUID a" "$expa" "$uida"

	/tmp/batcher delete -concurrency 4 -database batchertestdb -dbtype postgres -host localhost -opts $opts -password btest -portnum $portnum -table uuidtest -user btest -where "intcol<101" -execute

	uidtot=$( $SQLCMD "SELECT COUNT(1) FROM uuidtest;" | grep -iv count | grep -iv row | tr -d '\r' )
	comp "Small delete UUID total" "$exptot" "$uidtot"

	/tmp/batcher update -concurrency 4 -database batchertestdb -dbtype postgres -host localhost -opts $opts -password btest -portnum $portnum -set "strcol='b'"  -table compositetest -user btest -where "strcol='a'" -execute

	cmpa=$( $SQLCMD "SELECT COUNT(1) FROM compositetest WHERE strcol = 'a';" | grep -iv count | grep -iv row | tr -d '\r' )
	comp "Updated composite a" "$expa" "$cmpa"

	/tmp/batcher delete -concurrency 4 -database batchertestdb -dbtype postgres -host localhost -opts $opts -password btest -portnum $portnum -table compositetest -user btest -where "intcol<101" -execute
	...
	/tmp/batcher delete -concurrency 4 -database batchertestdb -dbtype postgres -host localhost -opts $opts -password btest -portnum $portnum -table serialtest -user btest -where "1=1" -execute

	sertot=$( $SQLCMD "SELECT COUNT(1) FROM serialtest;" | grep -iv count | grep -iv row | tr -d '\r' )
	comp "Full delete serial total" "$exptot" "$sertot"

	/tmp/batcher delete -concurrency 4 -database batchertestdb -dbtype postgres -host localhost -opts $opts -password btest -portnum $portnum -table uuidtest -user btest -where "1=1" -execute

	uidtot=$( $SQLCMD "SELECT COUNT(1) FROM uuidtest;" | grep -iv count | grep -iv row | tr -d '\r' )
	comp "Full delete UUID total" "$exptot" "$uidtot"

	/tmp/batcher delete -concurrency 4 -database batchertestdb -dbtype postgres -host localhost -opts $opts -password btest -portnum $portnum -table compositetest -user btest -where "1=1" -execute

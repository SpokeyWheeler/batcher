...
SQLCMD0="cockroach sql --url=postgres://$DBUSER:$DBPASSWORD@localhost:26257/postgres?sslmode=verify-full&sslrootcert=/tmp/certs/ca.crt&sslcert=/tmp/certs/client.$DBUSER.crt&sslkey=/tmp/certs/client.$DBUSER.key "
SQLCMD="cockroach sql --url=postgres://$DBUSER:$DBPASSWORD@localhost:26257/postgres?sslmode=verify-full&sslrootcert=/tmp/certs/ca.crt&sslcert=/tmp/certs/client.$DBUSER.crt&sslkey=/tmp/certs/client.$DBUSER.key --format tsv -e "
...
../batcher update -concurrency 4 -database batchertestdb -dbtype postgres -host localhost -opts "sslmode=disable" -password $DBPASSWORD -portnum 26257 -set "strcol='b'" -user DBUSER -where "strcol='a'" -execute
